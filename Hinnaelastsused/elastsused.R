...
plot(ts.union(diisel,skp.sa,hind,hind/hind_lt, veoautod))
...
lt.ecm <- lm(ldiisel ~ lskp + lhind + lt_diff + log(Trend))
summary(lt.ecm)

long.ecm <- lm(ldiisel ~ lskp + lhind + lt_diff + log(Trend))
summary(long.ecm)
...
summary(dynlm(d(ldiisel) ~ d(lskp) + d(lhind) + L(ldiisel,1) + L(lhind,1) + L(lskp,1) + L(lt_diff,1) + L(log(Trend), 1)))

ecm.resid = ts(long.ecm$residuals, start=(c(2005,1)), end=c(2021,1), frequency = 4)

short.ecm <- dynlm(d(ldiisel) ~ d(lskp) + d(lt_diff) + d(lhind) +L(ecm.resid,1))
AIC(short.ecm)
summary(short.ecm)
stargazer(short.ecm)
summary(test.ecm)
...
a <- ts.union(d.predixt, diff(ldiisel_v))
a <- as.data.frame(a)
a <- setNames(a, c('f', 'e'))
